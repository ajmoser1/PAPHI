-- Update search_members for multi-tenant scope, privacy, and ghost exclusion

DROP FUNCTION IF EXISTS search_members(text, uuid, uuid, boolean, int, int);
DROP FUNCTION IF EXISTS search_members(text, uuid, uuid, int, int);
...
-- search_alumni wrapper (alumni-only filter)
...
SET search_path = public
AS $$
  SELECT * FROM search_members(
    search_query,
    filter_industry_id,
    filter_company_id,
    true,
    filter_fraternity_id,
    filter_chapter_id,
    viewer_chapter_id,
    result_limit,
    result_offset
...
GRANT EXECUTE ON FUNCTION search_alumni(text, uuid, uuid, uuid, uuid, uuid, int, int) TO authenticated;
